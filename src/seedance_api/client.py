...
class SeedanceClient:
    """Minimal Python wrapper for CyberBara Seedance endpoints."""
    ...
    @staticmethod
    def full_access_models() -> tuple[str, ...]:
        return FULL_ACCESS_MODELS
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise SeedanceAPIError(message, status_code=exc.code, payload=payload) from exc
        except error.URLError as exc:
            raise SeedanceAPIError(f"Request failed: {exc.reason}") from exc
    ...
    def _build_multipart_upload(file_paths: list[str]) -> tuple[bytes, str]:
        boundary = f"----SeedanceBoundary{uuid.uuid4().hex}"
        chunks: list[bytes] = []
